...
@testset "NewtonsMethod.jl" begin
    ...
    @test NewtonsMethod.newtonroot(f, df; maxiter = 2) === nothing
    @test NewtonsMethod.newtonroot(f; maxiter = 2) === nothing

    @test NewtonsMethod.newtonroot(f, df; maxiter = 200) ≈ 1.0
    @test NewtonsMethod.newtonroot(f; maxiter = 200) ≈ 1.0
    ...
    @test !(NewtonsMethod.newtonroot(f, df; tol = 0.1) ≈ 1.0)
    @test !(NewtonsMethod.newtonroot(f; tol = 0.1) ≈ 1.0)
    ...
    @test NewtonsMethod.newtonroot(f, df; iv = 0.7) ≈ 1.0
    @test NewtonsMethod.newtonroot(f; iv = 0.7) ≈ 1.0
